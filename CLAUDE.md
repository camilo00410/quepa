...
- **Zona canónica (`city_zones`, change `add-console-zone-selector` / backend `add-city-zones`, migración `0010`)**: la zona se captura por **selector** poblado de `city_zones` (activas de la ciudad efectiva — `location.city ?? ciudad del lugar`) y se guarda como `place_locations.zone_id`. El Console **jamás** escribe el campo legado `zone` (texto deprecado, solo fallback de lectura: canónico si hay FK, legado si no — regla en `renderBarrioSelect`/`fillZoneSelect`), **jamás** inserta filas en `city_zones` (el gazetteer se cura en el webhook: `pnpm zones:discover` → `data/zones/*.json` → `pnpm zones:apply`). Una sede solo-legado muestra `"…" · texto legado` como opción no seleccionable; la transición a FK la hace el staff eligiendo del selector. El **centroide** de la zona centra el mapa como referencia aproximada (pin `.approx`) y NUNCA se persiste en `lat`/`lng`. Sin zonas para la ciudad (gazetteer vacío o query fallida): selector deshabilitado con nota, el guardado del resto no se bloquea, y no se reabre texto libre.
